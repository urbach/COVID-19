...
## 52 is 20
bonn[c(40:length(ii))] <- c(1, 1, 1, 1, 1, 1, 2, 4, 5, 6, 13, 18, 20, 36, 37, 55, 64, 76, 92, 119, 132, 151, 161, 180, 205, 222, 241, 259, 268, 272, 294, 294, 353, 370, 395, 412, 416, 417, 447, 474, 484, 504, 518, 530, 539, 550, 552, NA)
...
Als drittes Fitfenster gibt es mittlerweile von Tag 59 bis heute.
...
legend("topleft",
       ...
       pch=c(21, NA, NA, NA), lty=1)
legend("bottomright",
       legend=c("Schulen geschl.", "Lockdown"),
       lty=c(2,3), bty="n")
...
Mitlerweile sinkt die Anzahl akut Infizierter stetig.
...
durch die Anzahl akut Infizierter vom Vortag teilt. Das sieht dann so
aus
...
plot(y=as.vector(colSums(xdata[, ii])), x=x,
     log="y", type='l', ylim=c(50,1500000),
     xlab=c(""), ylab="Infektionen", col="red", lwd=1.5, xaxt="n")
dateaxis(datestring)
lines(y=as.vector(colSums(xdata[grepl("China", x=xdata$Country.Region), ii])),
      x=x, col="blue", lty=2, lwd=1.5)
